 ...
 **/
let cal_bgColor = event_api.ect_cal_bgColor;
let cal_textColor = event_api.ect_cal_textColor;
...
const ect_templates = {
    ...
    popupDetailDate: function(isAllDay, start, end) {
        ...
        let date_format = jQuery('.ect-custom-calendar').attr('data-date-format');
        let time_format = jQuery('.ect-custom-calendar').attr('data-time-format');
    ...
    popupDetailBody: function(schedule) {
        let desc = unescape(jQuery(schedule.body).text()); // remove all HTML
        if (desc.length < 3) {
            return;
        }
        if (desc.length > 255) {
...
(function($) {
    ...
    $(calendar_container).each(function(key, value) {
            ...
            function ect_render_events() {
                ...
                let cal = $('.ect-calendar-select[data-calendar-id="' + ID + '"]');
                let view = $(cal).find(":selected").val();
                if (view != 'month') {
                    ...
                    success: function(res) {
                        ...
                        for (let i = 0; i < events.length; i++) {
                            ...
                            if (events[i]['categories'].length != 0) {
                                let evt_end_date = new Date(events[i]['end_date']);
                                let evt_start_date = new Date(events[i]['start_date']);
                                let dateStart = calendar[ID].getDateRangeStart();
                                let dateEnd = calendar[ID].getDateRangeEnd();
                                let calendar_start_date = dateStart.getFullYear() + "-" + (dateStart.getMonth() + 1) + "-" + dateStart.getDate();
                                let calendar_end_date = dateEnd.getFullYear() + "-" + (dateEnd.getMonth() + 1) + "-" + dateEnd.getDate();
                                ev_category = events[i]['categories'][0]['name'];
                                if (ev_category != '' && category.indexOf(ev_category) == -1 && (
                                        (Date.parse(evt_end_date.toDateString()) >= Date.parse(calendar_start_date) &&
                                            Date.parse(evt_end_date.toDateString()) <= Date.parse(calendar_end_date)
                                        ) ||
                                        (Date.parse(evt_start_date.toDateString()) >= Date.parse(calendar_start_date) &&
                                            Date.parse(evt_start_date.toDateString()) <= Date.parse(calendar_end_date)
                                        )
                                    )) {
                            ...
                            let custom_css = 'background-color:' + evt_bgColor + ';color:' + evt_textColor + ';'
                            Schedule.push({
                                id: i + 1,
                                ...
                                end: new Date(events[i]['end_date'])
                            });
                        }
            ...
            $('.ect-calendar-select[data-calendar-id="' + ID + '"]').on('change', function() {
                let view = $(this).find(":selected").val();
                //calendar[ID].render();
            ...
            $(document).on('click', ".ect-calendar-cat-filter[data-calendar-id='" + ID + "'] input[type='checkbox']", function() {
                let cat_name = $(this).attr('data-event-cat');
                let option = $(this).is(":checked") ? false : true;
                if (option == true) {
            ...
            calendar[ID].on('clickSchedule', function(event) {
                let event_url = event.schedule.raw;
                let start = new Date(event.schedule.raw[2]).getTime();
                let end = new Date(event.schedule.raw[3]).getTime();
                let all_day_event = event.schedule.raw[4];
                let date_format = jQuery('.ect-custom-calendar').attr('data-date-format');
                let time_format = jQuery('.ect-custom-calendar').attr('data-time-format');
                ...
                let endFormat = (isSameDate ? '' : date_format) + ' ' + time_format;
                let date_time_data = '';
                if (all_day_event) {
                ...
                let popup_title = $('.tui-full-calendar-popup-detail .tui-full-calendar-schedule-title').text();
            ...
            $('.ect-calendar_btn[data-calendar-id="' + ID + '"]').on('click', function() {
                let action = $(this).attr('data-action');
                let cal_id = $(this).attr('data-calendar-id');
                switch (action) {
                    case 'move-today':
