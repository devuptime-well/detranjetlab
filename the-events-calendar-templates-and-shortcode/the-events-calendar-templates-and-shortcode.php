...
if (!defined('ECT_VERSION_CURRENT')){
	define('ECT_VERSION_CURRENT', '2.1.1');
}
...
if (!class_exists('EventsCalendarTemplates')) {
	...
	public function ect_load_files() {
		...
		require_once(ECT_PLUGIN_DIR.'admin/ect-pro-settings.php' );
		require_once(ECT_PLUGIN_DIR.'/templates/calendar-template.php');
		/*** Plugin review notice file */ 
		require_once(ECT_PLUGIN_DIR.'admin/ect-feedback-notice.php');
		if( is_admin()){
			require_once(ECT_PLUGIN_DIR .'admin/init-api.php');
		}
    	...
    	if ( !function_exists( 'tribe_get_events' ) ) {
			...
			ect_load_assets($template,$style,$slider_pp_id,$autoplay, $carousel_slide_show);
				...
				if($socialshare=="yes"){
					wp_enqueue_script('ect-sharebutton',ECT_PLUGIN_URL .'assets/js/ect-sharebutton.js',array('jquery'),null,true);
					wp_enqueue_style('ect-sharebutton-css',ECT_PLUGIN_URL .'assets/css/ect-sharebutton.css',null, null,'all');
				}
			/* 
			Build ECT query
			*/
			...
			*/
			...
			if ( $all_events ) {
				...
				$excludePosts[]=$event_id;
				 ...
				 if ( $show_headers ) {
					...
					if ($prev_event_month != $event_month || ( $prev_event_month == $event_month && $prev_event_year != $event_year ) ) {		
						...
						$date_header= sprintf( "<span class='month-year-box'><span>%s</span></span>", tribe_get_start_date( $post, false, $month_year_format ) );
				...
				if(in_array($template,array("timeline","classic-timeline",'timeline-view'))) {
					...
					include(ECT_PLUGIN_DIR.'/templates/slider-template.php');	
				}
				else if($template=="grid-view") {
					$ect_grid_columns=$attribute['columns'];
					$grid_style=$attribute['style'];
					$hide_venue=$attribute['hide-venue'];
					include(ECT_PLUGIN_DIR.'/templates/grid-template.php');	
				}
				else if(in_array($template,array("carousel-view"))) {
					include(ECT_PLUGIN_DIR.'/templates/carousel-template.php');	
				}else if(in_array($template,array("masonry-view"))){
						$grid_style=$attribute['style'];
						$ect_grid_columns=$attribute['columns'];
						include(ECT_PLUGIN_DIR.'/templates/masonry-template.php');	
				}
				else if($template=="accordion-view") {
					$ect_compare='';
					include(ECT_PLUGIN_DIR.'/templates/accordion-template.php');	
				}
			...
			// create wrapper elements for all layouts
				...
				$output .='<div class="ect-load-more '.$style.'">
					...
					<img class="ect-preloader" style="display:none;" src="'.ECT_PLUGIN_URL.'assets/images/preloader.svg"> <span class="ect-btn-text">'.__('Load More','ect').'</span></a>
				...
				*/
				...
				if ( $all_events && $total_events>$attribute['limit']) {
					...
					<section data-exclude-events="'.json_encode($excludePosts).'"  id="ect-lm-settings" data-load-more="'.__('Load more','ect').'"  data-loaded="'.__('No Event Found','ect').'" 
					data-loading="'.__('Loading','ect').'" data-settings='.json_encode($settings).' data-ajax-url="'.admin_url('admin-ajax.php').'">
					<div id="ect-cat-load-more" style="display:none;"><img class="ect-preloader"  src="'.ECT_PLUGIN_URL.'assets/images/preloader.svg"> <span class="ect-btn-text"></span></div>
					</section>
					<script type="application/json" id="ect-query-arg">'.json_encode($ect_args).'</script></div>';
					}	
				}
...
function ect_common_loadmore_handler(){
	...
	$excludePosts=array();
	include(ECT_PLUGIN_DIR.'/includes/ect-load-more-handler.php' );
...
function ect_catfilters_load_more(){
	...
	if($cat==""){
		unset($ect_args['tax_query']);
			...
			include(ECT_PLUGIN_DIR.'/includes/ect-masonry-loop.php' );
			echo json_encode($response);
			wp_die();
}
