...
namespace ECTREG;
	class Ectbase {
    	public $key = "E16454D2F2A55C19";
    	private $product_id = "2";
    	private $product_base = "ect";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_ect_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=ect_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
					}
