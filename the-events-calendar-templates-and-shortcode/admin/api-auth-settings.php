...
*/
    class ECT_Settings
    {
        ...
        public function __construct()
        {
            ...
            add_action('admin_menu', array($this, 'admin_menu'),100,0);
        ...
        */
        ...
        {
            add_submenu_page('edit.php?post_type=tribe_events','Cool Plugins Product Registration', 'Shortcode & Template Registration', 'delete_posts', $this->AUTH_PAGE, array($this, 'auth_settings_page'));
        }
        ...
        */
        public function ValidatePurchase()
        {
           ...
           $test = new Ectbase($this->Base_File);
           if( isset( $_GET['settings-updated'] ) || !empty($purchase) )
           {
                if( !empty( $purchase ) ){
                    ...
                    if( $verified && strcmp($this->Response->support_end,'No support') >0 ){
        ...
        function admin_registration_notice(){
            ...
            ?>
                <div class="license-warning notice notice-error is-dismissible">
                    <p>Hi, <strong><?php echo ucwords( $user_name ); ?></strong>! Please <strong><a href="<?php echo esc_url( get_admin_url( null, 'edit.php?post_type=tribe_events&page='.$this->AUTH_PAGE.'#'.$this->PREFIX.'_license_registration' ) ); ?>">enter and activate</a></strong> your license key for <strong><?php echo $this->PLUGIN_NAME;?></strong> plugin for unrestricted and full access of all premium features.</p>
                </div>
            <?php
        }
